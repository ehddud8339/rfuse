...
#define RFUSE_PATH_LATENCY_MAX_SAMPLES 1000000U
#define RFUSE_PATH_LATENCY_SUBMIT_BITS 10
#define RFUSE_PATH_LATENCY_SUBMIT_BUCKETS (1U << RFUSE_PATH_LATENCY_SUBMIT_BITS)

struct rfuse_path_latency_series {
	const char *name;
	raw_spinlock_t lock;
	u64 *samples;
	u32 head;
...
struct rfuse_path_latency_submit {
	struct hlist_node node;
	struct rfuse_req *r_req;
	u64 submit_ns;
};

static struct rfuse_path_latency_series rfuse_path_latency_series[] = {
	[RFUSE_PATH_LATENCY_READPAGES_GET_REQ] = {
		.name = "readpages_get_req",
	},
	[RFUSE_PATH_LATENCY_READPAGES_ARGS_FILL] = {
		.name = "readpages_args_fill",
	},
	[RFUSE_PATH_LATENCY_READPAGES_PREPARE_PAYLOAD] = {
		.name = "readpages_prepare_payload",
	},
	[RFUSE_PATH_LATENCY_READPAGES_SIMPLE_BACKGROUND] = {
		.name = "readpages_simple_background",
	},
	[RFUSE_PATH_LATENCY_READPAGES_SIMPLE_REQUEST] = {
		.name = "readpages_simple_request",
	},
	[RFUSE_PATH_LATENCY_READPAGES_END] = {
		.name = "readpages_end",
	},
	[RFUSE_PATH_LATENCY_QUEUE_TO_END] = {
		.name = "queue_to_request_end",
	},
	[RFUSE_PATH_LATENCY_END_TO_COMPLETION] = {
		.name = "request_end_to_completion_end",
	},
};
static struct hlist_head rfuse_path_latency_submits[RFUSE_PATH_LATENCY_SUBMIT_BUCKETS];
static raw_spinlock_t rfuse_path_latency_submit_lock;

static u32 rfuse_path_latency_submit_hash(struct rfuse_req *r_req)
{
	return hash_ptr(r_req, RFUSE_PATH_LATENCY_SUBMIT_BITS);
}

static int rfuse_path_latency_u64_cmp(const void *lhs, const void *rhs)
{
...
{
	struct rfuse_path_latency_series *series;
	unsigned long flags;
	u32 head;

	if (id >= RFUSE_PATH_LATENCY_COUNT)
		return;

	series = &rfuse_path_latency_series[id];
	if (unlikely(!series->samples))
		return;

	raw_spin_lock_irqsave(&series->lock, flags);
	head = series->head;
	series->samples[head] = latency_ns;
	head++;
	if (head == RFUSE_PATH_LATENCY_MAX_SAMPLES)
		head = 0;
	series->head = head;
	if (series->count < RFUSE_PATH_LATENCY_MAX_SAMPLES)
		series->count++;
	raw_spin_unlock_irqrestore(&series->lock, flags);
}

void rfuse_path_latency_submit_start(struct rfuse_req *r_req, u64 submit_ns)
{
	struct rfuse_path_latency_submit *submit;
	unsigned long flags;
	u32 bucket;

	submit = kmalloc(sizeof(*submit), GFP_ATOMIC);
	if (!submit)
		return;

	submit->r_req = r_req;
	submit->submit_ns = submit_ns;

	bucket = rfuse_path_latency_submit_hash(r_req);
	raw_spin_lock_irqsave(&rfuse_path_latency_submit_lock, flags);
	hlist_add_head(&submit->node, &rfuse_path_latency_submits[bucket]);
	raw_spin_unlock_irqrestore(&rfuse_path_latency_submit_lock, flags);
}

static bool rfuse_path_latency_submit_take(struct rfuse_req *r_req,
					   u64 *submit_ns)
{
	struct rfuse_path_latency_submit *submit;
	struct hlist_node *tmp;
	unsigned long flags;
	u32 bucket = rfuse_path_latency_submit_hash(r_req);
	bool found = false;

	raw_spin_lock_irqsave(&rfuse_path_latency_submit_lock, flags);
	hlist_for_each_entry_safe(submit, tmp, &rfuse_path_latency_submits[bucket],
				  node) {
		if (submit->r_req != r_req)
			continue;
		*submit_ns = submit->submit_ns;
		hlist_del(&submit->node);
		found = true;
		break;
	}
	raw_spin_unlock_irqrestore(&rfuse_path_latency_submit_lock, flags);
...
static u64 *rfuse_path_latency_snapshot_and_reset(
	struct rfuse_path_latency_series *series, u32 *nr_samples)
{
	unsigned long flags;
	u64 *snapshot;
	u32 count;
	...
	*nr_samples = 0;
	...
	snapshot = vmalloc(array_size(RFUSE_PATH_LATENCY_MAX_SAMPLES,
				      sizeof(*snapshot)));
	if (!snapshot)
	...
	if (count == RFUSE_PATH_LATENCY_MAX_SAMPLES) {
		first_chunk = RFUSE_PATH_LATENCY_MAX_SAMPLES - series->head;
		memcpy(snapshot, &series->samples[series->head],
		       first_chunk * sizeof(*snapshot));
		second_chunk = series->head;
		if (second_chunk)
			memcpy(snapshot + first_chunk, series->samples,
			       second_chunk * sizeof(*snapshot));
	} else {
		memcpy(snapshot, series->samples, count * sizeof(*snapshot));
	}
...
static u64 rfuse_path_latency_percentile(u64 *samples, u32 count,
...
static void rfuse_path_latency_dump_series(
	struct rfuse_path_latency_series *series, u64 *samples, u32 count)
{
	u64 min_ns;
	u64 max_ns;
	u64 avg_ns;
	...
	sort(samples, count, sizeof(*samples), rfuse_path_latency_u64_cmp, NULL);
	...
	p50 = rfuse_path_latency_percentile(samples, count, 50, 100);
	p90 = rfuse_path_latency_percentile(samples, count, 90, 100);
	p95 = rfuse_path_latency_percentile(samples, count, 95, 100);
	p99 = rfuse_path_latency_percentile(samples, count, 99, 100);
	p999 = rfuse_path_latency_percentile(samples, count, 999, 1000);

	pr_info("rfuse path latency %s: samples=%u avg=%llu ns min=%llu ns max=%llu ns p50=%llu ns p90=%llu ns p95=%llu ns p99=%llu ns p99.9=%llu ns\n",
		series->name, count, (unsigned long long)avg_ns,
...
int rfuse_path_latency_stats_init(void)
{
	...
	raw_spin_lock_init(&rfuse_path_latency_submit_lock);
	for (i = 0; i < RFUSE_PATH_LATENCY_SUBMIT_BUCKETS; i++)
		INIT_HLIST_HEAD(&rfuse_path_latency_submits[i]);

	for (i = 0; i < RFUSE_PATH_LATENCY_COUNT; i++) {
		struct rfuse_path_latency_series *series =
			&rfuse_path_latency_series[i];

		raw_spin_lock_init(&series->lock);
		series->samples = vmalloc(array_size(RFUSE_PATH_LATENCY_MAX_SAMPLES,
						      sizeof(*series->samples)));
		if (!series->samples)
			goto err_destroy;
		series->head = 0;
		series->count = 0;
...
err_destroy:
	for (j = 0; j < i; j++) {
		vfree(rfuse_path_latency_series[j].samples);
		rfuse_path_latency_series[j].samples = NULL;
	}
	return -ENOMEM;
}
...
void rfuse_path_latency_stats_destroy(void)
{
	...
	for (i = 0; i < RFUSE_PATH_LATENCY_COUNT; i++) {
		vfree(rfuse_path_latency_series[i].samples);
		rfuse_path_latency_series[i].samples = NULL;
		rfuse_path_latency_series[i].head = 0;
		rfuse_path_latency_series[i].count = 0;
	}

	raw_spin_lock_irqsave(&rfuse_path_latency_submit_lock, flags);
	for (i = 0; i < RFUSE_PATH_LATENCY_SUBMIT_BUCKETS; i++) {
		hlist_for_each_entry_safe(submit, tmp,
					  &rfuse_path_latency_submits[i],
					  node) {
			hlist_del(&submit->node);
			kfree(submit);
		}
...
void rfuse_path_latency_stats_dump_and_reset(void)
{
	...
	for (i = 0; i < RFUSE_PATH_LATENCY_COUNT; i++) {
		...
		snapshot = rfuse_path_latency_snapshot_and_reset(series,
								 &nr_samples);
		if (IS_ERR(snapshot)) {
			pr_info("rfuse path latency %s: snapshot allocation failed: %ld\n",
				series->name, PTR_ERR(snapshot));
			continue;
		}
		if (!snapshot) {
			pr_info("rfuse path latency %s: no samples collected\n",
				series->name);
			continue;
		}

		rfuse_path_latency_dump_series(series, snapshot, nr_samples);
		vfree(snapshot);
	}
}
...
static void rfuse_queue_request(struct rfuse_req *r_req){
  ...
  // [DEBUG] submit request
	rfuse_path_latency_submit_start(r_req, ktime_get_ns());
...
void rfuse_request_end(struct rfuse_req *r_req){
	...
	u64 request_end_ns;
	u64 submit_ns;
  // [DEBUG] request end(daemon work end)
	request_end_ns = ktime_get_ns();
	if (rfuse_path_latency_submit_take(r_req, &submit_ns))
		rfuse_path_latency_record(RFUSE_PATH_LATENCY_QUEUE_TO_END,
					  request_end_ns - submit_ns);
	GET_TIMESTAMPS(6)
	if(test_bit(FR_BACKGROUND, &r_req->flags)){
		spin_lock(&riq->bg_lock);
	...
	rfuse_put_request(r_req);
  // [DEBUG] request completion end
	rfuse_path_latency_record(RFUSE_PATH_LATENCY_END_TO_COMPLETION,
				  ktime_get_ns() - request_end_ns);
}
