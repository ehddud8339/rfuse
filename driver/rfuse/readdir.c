...
static bool rfuse_use_readdirplus(struct inode *dir, struct dir_context *ctx)
{
...
static void rfuse_readdir_cache_end(struct file *file, loff_t pos)
{
	struct fuse_inode *fi = get_fuse_inode(file_inode(file));
	loff_t end;
...
int rfuse_readdir_uncached(struct file *file, struct dir_context *ctx)
{
	...
	if (plus) {
		...
		rfuse_read_args_fill(&ria, file, ctx->pos, PAGE_SIZE,
				    FUSE_READDIRPLUS);
	} else {
		rfuse_read_args_fill(&ria, file, ctx->pos, PAGE_SIZE,
				    FUSE_READDIR);
	...
	if (res >= 0) {
		if (!res) {
			...
			if (ff->open_flags & FOPEN_CACHE_DIR)
				rfuse_readdir_cache_end(file, ctx->pos);
		} else if (plus) {
...
{
	return rfuse_readdir_uncached(file,ctx);
	// int plus;
	// ssize_t res;
	// struct page *page;
	// struct inode *inode = file_inode(file);
