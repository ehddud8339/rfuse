...
struct fuse_file *fuse_file_open(struct fuse_mount *fm, u64 nodeid,
	...
	return rfuse_file_open(fm,nodeid,open_flags,isdir);
...
{
	rfuse_file_release(inode,ff,open_flags,id,isdir);
	// struct fuse_inode *fi = get_fuse_inode(inode);
	// struct fuse_release_args *ra = ff->release_args;
...
static int fuse_fsync(struct file *file, loff_t start, loff_t end,
		      int datasync)
{
	return rfuse_fsync(file,start,end,datasync);
// 	struct inode *inode = file->f_mapping->host;
// 	struct fuse_conn *fc = get_fuse_conn(inode);
// 	int err;
...
static int fuse_do_readpage(struct file *file, struct page *page){
	return rfuse_do_readpage(file,page);
...
static ssize_t fuse_perform_write(struct kiocb *iocb,
	...
	return rfuse_perform_write(iocb, mapping,ii,pos);
...
{
	return rfuse_direct_read_iter(iocb, to);
	/*
	ssize_t res;
...
{
	return rfuse_direct_write_iter(iocb, from);
	/*
...
{
	return rfuse_write_inode(inode, wbc);
/* 	struct fuse_inode *fi = get_fuse_inode(inode);
	struct fuse_file *ff;
	int err;
...
{
	return rfuse_writepages(mapping, wbc);
	/*
	struct inode *inode = mapping->host;
	struct fuse_conn *fc = get_fuse_conn(inode);
...
static long fuse_file_fallocate(struct file *file, int mode, loff_t offset,
				loff_t length)
{
	return rfuse_file_fallocate(file, mode, offset, length);
	/*
	struct fuse_file *ff = file->private_data;
	struct inode *inode = file_inode(file);
	struct fuse_inode *fi = get_fuse_inode(inode);
