...
{
	printk("Fuse lookup called!, but this should not be called...\n");
...
static struct dentry *fuse_lookup(struct inode *dir, struct dentry *entry,
				  unsigned int flags)
{
	return rfuse_lookup(dir,entry,flags);
// 	int err;
// 	struct fuse_entry_out outarg;
// 	struct inode *inode;
...
static int fuse_create_open(struct inode *dir, struct dentry *entry,
			    ...
			    umode_t mode){
	return rfuse_create_open(dir,entry,file,flags,mode);
// 	int err;
// 	struct inode *inode;
			...
			struct dentry *newent, unsigned int flags)
{
	...
	if (flags) {
		...
		err = rfuse_rename_common(olddir, oldent, newdir, newent, flags,
					 FUSE_RENAME2,
		...
		err = rfuse_rename_common(olddir, oldent, newdir, newent, 0,
					 FUSE_RENAME,
...
/*
	...
	return err;
*/
	return rfuse_link(entry, newdir, newent);
}
...
static int fuse_do_getattr(struct inode *inode, struct kstat *stat,
			   struct file *file)
{
	return rfuse_do_getattr(inode,stat,file);
	// int err;
...
{
	return rfuse_access(inode, mask);
	// struct fuse_mount *fm = get_fuse_mount(inode);
	// FUSE_ARGS(args);
...
static int fuse_readlink_page(struct inode *inode, struct page *page)
{
	...
	return 0;
	*/
	return rfuse_readlink_page(inode, page);
}
			  ...
			  int datasync)
{
	...
	inode_lock(inode);
	err = rfuse_fsync_common(file, start, end, datasync, FUSE_FSYNCDIR);
	if (err == -ENOSYS) {
		fc->no_fsyncdir = 1;
		err = 0;
	}
...
int fuse_do_setattr(struct dentry *dentry, struct iattr *attr,
		    struct file *file){
	return rfuse_do_setattr(dentry,attr,file);
