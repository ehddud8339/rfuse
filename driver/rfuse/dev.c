...
/*
	Initialize a request data structure, the flag is set to pending at first
*/
...
/*
	Allocate a pre cached request structure and initialize it
*/
...
/*
	Return a request to the request structure cache
*/
...
/* 
	Return true only if..
	- not initialized
	- Blocked & The request is for a background 
*/
...
{
	rfuse_queue_forget(fc,nodeid, nlookup);
}

/*
	Flush bg queue
	- Do the following steps while
	(The active background requests are smaller than the maximum background requests)
	(This backround queue is not empty)
		- get the first entry of the background queue
		- delete it from the background queue
...
ssize_t fuse_simple_request(struct fuse_mount *fm, struct fuse_args *args)
{
	...
	// Forcse set --> allocation policy is more strict
	if (args->force) {
		atomic_inc(&fc->num_waiting);
	...
	fuse_adjust_compat(fc, args); // for compatibility
	fuse_args_to_req(req, args); // Generate request from arguments

	// If this request does noe require any reply...
	if (!args->noreply)
		__set_bit(FR_ISREPLY, &req->flags);
	
	__fuse_request_send(req);
	ret = req->out.h.error; // return the out.error
		...
		struct fuse_copy_state *cs, size_t nbytes)
{
	 ...
	 * for header room.
	 */
	// printk("size of a read buffer is %ld\n",nbytes); // 1052672
...
restart:
	for (;;) {
		...
		if (file->f_flags & O_NONBLOCK)
			return -EAGAIN;
		// WAITS until a EVENT is available!!!!!!
		// If Interrupt, forget, pending queue is not empty... it wakes up
	...
	// then process forget pending queue
	if (forget_pending(fiq)) {
		if (list_empty(&fiq->pending) || fiq->forget_batch-- > 0)
			return fuse_read_forget(fc, fiq, cs, nbytes);

		if (fiq->forget_batch <= -8) // to select fairly between forget/non-forget request
			fiq->forget_batch = 16;
	}

	// Finally process pending queue
	...
	/*
		...
		This code is for FUSE RFUSE compatability, should be removed eventually

	*/
	if(req->in.h.opcode == FUSE_GETATTR || req->in.h.opcode == FUSE_SETATTR || req->in.h.opcode == FUSE_LOOKUP || 
		req->in.h.opcode == FUSE_RMDIR || req->in.h.opcode == FUSE_MKDIR || req->in.h.opcode == FUSE_STATFS ||
		req->in.h.opcode == FUSE_FLUSH || req->in.h.opcode == FUSE_FSYNC || req->in.h.opcode == FUSE_CREATE ||
		req->in.h.opcode == FUSE_OPEN || req->in.h.opcode == FUSE_OPENDIR || req->in.h.opcode == FUSE_RELEASE ||
		req->in.h.opcode == FUSE_RELEASEDIR || req->in.h.opcode == FUSE_UNLINK || req->in.h.opcode == FUSE_INIT ||
		req->in.h.opcode == FUSE_WRITE || req->in.h.opcode == FUSE_READ || req->in.h.opcode == FUSE_READDIR){
		
		args_temp.opcode = req->in.h.opcode;
		args_temp.in_numargs = 0;
		args_temp.out_numargs = 0;
		args = &args_temp;
	}
	else
		args = req->args;
...
static ssize_t fuse_dev_read(struct kiocb *iocb, struct iov_iter *to)
{
	...
	else // USER DATA READ COMES TO here during (Write)
		return rfuse_dev_do_read(fud, file,to, iov_iter_count(to), iocb->ki_pos);
}
		...
		struct fuse_copy_state *cs, size_t nbytes)
{
	...
	err = fuse_copy_one(cs, &oh, sizeof(oh)); // copy from write buffer
	...
	req = NULL;
	if (fpq->connected)
		req = request_find(fpq, oh.unique & ~FUSE_INT_REQ_BIT); // find the request
	...
	if (oh.error)
		...
		err = copy_out_args(cs, req->args, nbytes); // copy arguments
	fuse_copy_finish(cs);
...
static ssize_t fuse_dev_write(struct kiocb *iocb, struct iov_iter *from)
{
	...
	/**
	 * RFUSE KHJ COMMENT
	 * Need to distinguish between "RFUSE USER DATA WRITE" and 
	 * "FUSE DEVICE WRITE"
	 * - "RFUSE USER DATA WRITE": writing the user data to the kernel space page
	 * - "FUSE DEVICE WRITE": after processing a request, this is how the
	 * request is finished. Writing to the device.
	 */
	

	if(iocb->ki_pos == 1000000){
		fuse_copy_init(&cs, 0, from);
		return fuse_dev_do_write(fud, &cs, iov_iter_count(from));
	}
	else
		return rfuse_dev_do_write(fud,from, iov_iter_count(from), iocb->ki_pos);
...
void fuse_abort_conn(struct fuse_conn *fc)
{
	...
	if (fc->connected) {
		...
		spin_unlock(&fiq->lock);
		rfuse_abort_conn(fc); // disconnect rfuse iqueues and related threads
		kill_fasync(&fiq->fasync, SIGIO, POLL_IN);
		end_polls(fc);
		wake_up_all(&fc->blocked_waitq);
		spin_unlock(&fc->lock);
		...
		unsigned long arg)
{
	...
	switch (cmd) {
		case FUSE_DEV_IOC_CLONE:
			...
			break;
		case RFUSE_WAKE_UP_COMP:
			fud = fuse_get_dev(file);
			res = -EFAULT;
			if(copy_from_user(&args, (char __user *)arg, sizeof(struct ioctl_args)))
				break;

			// printk("RFUSE: Start Wake up completion threads from user, riq_id: %d, req_index: %d\n", args.riq_id, args.req_index);
			res = 0;
 ...
 */
static int rfuse_dev_mmap(struct file *file, struct vm_area_struct *vma){
	unsigned long size = (unsigned long)(vma->vm_end - vma->vm_start);
	unsigned long pfn;
	struct fuse_dev *fud = fuse_get_dev(file);	
	void *ptr;

	unsigned long long map_args = (vma->vm_pgoff) << PAGE_SHIFT;
	loff_t map_queue = map_args & RFUSE_QUEUE_MAP_MASK;
	int req_index = (int)((map_args & RFUSE_REQ_IDX_MASK) >> 32);
	int riq_id = (int)((map_args & RFUSE_RIQ_ID_MASK) >> 16);

	if (!fud){
		printk("rfuse mmap: no device\n");
		return -EPERM;
	}
	
	if(map_queue == RFUSE_WRITE || map_queue == RFUSE_READ){
		if(rfuse_io_mmap(vma, fud, req_index, riq_id, size)){
			printk("RFUSE : data mapping failed\n");
			return -EAGAIN;
		}
		else{
			return 0;
...
const struct file_operations fuse_dev_operations = {
	...
	.splice_write	= rfuse_dev_splice_write,
	.poll		= fuse_dev_poll,
